...
def main():
    ...
    incentivados_sem_filtro = st.Page(rentabilidade_inc_sem_filtro,title="Rentabilidade fundos (incentivados) - Sem filtro")
    calcula_rating = st.Page(rating_func,title="Rating Crédito Privado")
    st.set_page_config(layout='wide',page_title="Consulta CVM")
    pg = st.navigation([home,busca,rentabilidade,rentabilidade_incentivados,incentivados_sem_filtro,calcula_rating])
    pg.run()
...
def rentabilidade_inc_sem_filtro():
    st.title("Rentabilidade Fundos Incentivados Crédito Privado")
    st.sidebar.subheader("QUANTITAS | Compliance")
    d0 = datetime.today().date()
    mes4 = d0 - relativedelta(months=4)
    ...
    df_filtrada = fundos.informe_diario.inf_diario_fi[['CNPJ_FUNDO_CLASSE', 'DT_COMPTC', 'VL_QUOTA', 'VL_PATRIM_LIQ', 'NR_COTST']][fundos.informe_diario.inf_diario_fi.CNPJ_FUNDO_CLASSE.isin(lista_fundos_inc)]
    df_filtrada = df_filtrada.rename(columns={'CNPJ_FUNDO_CLASSE':'cnpj', 'DT_COMPTC':'data', 'VL_QUOTA':'cota', 'VL_PATRIM_LIQ':'pl', 'NR_COTST':'cotistas' })
...
def rating_func():
    ...
    with st.expander('Nova Análise'):
        with st.form('form-analise'):
            ...
            setor = st.selectbox("Setor",options=df_ratings_mod['Setor'].unique().tolist())
...
"""
    ...
    with st.expander('Pesos'):
        ...
        #renomear as linhas do df_pesos
        df_pesos['Métrica'] = df_pesos['Métrica'].replace({'controlador':'Controlador','dl_ebitda':'DL/EBITDA','margem_ebitda':'Margem EBITDA','fco_ebitda':'FCO/EBITDA',
            'ebitda_resultado_financeiro':'EBITDA/Resultado Financeiro','caixa_divida_cp':'Caixa/Dívida CP','roe':'ROE'})
        st.dataframe(
